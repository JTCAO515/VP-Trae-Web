...
export function LoginForm() {
  ...
  const nextPath = useMemo(() => searchParams.get('next') ?? '/drafts', [searchParams]);

  const [email, setEmail] = useState('operator@visepanda.local');
  ...
  return (
    <div className="min-h-screen bg-zinc-50 px-4 py-16">
      <div className="mx-auto w-full max-w-md rounded-xl border bg-white p-6">
        <h1 className="text-xl font-semibold">内容运营后台登录</h1>
        ...
        <div className="mt-6 rounded-lg border bg-zinc-50 p-3 text-xs text-zinc-700">
          ...
          <div className="mt-1">
            operator：<code>operator@visepanda.local / Operator123!</code>
          </div>
          <div>
